...
const url =
  "mongodb+srv://gokul:gokul2781@chatcatdb.odn9j.mongodb.net/OMDB?retryWrites=true&w=majority";
...
const updateMovies = async (id, data) => {
  const client = await connectToDb();
  const db = client.db("OMDB");
  db.collection("movies").updateOne({ imdbID: id }, { $set: data });
};

const deleteMovies = async (id) => {
  const client = await connectToDb();
  const db = client.db("OMDB");
  db.collection("movies").removeOne({ _id: ObjectId(id) });
};
